...
import { Plus, CreditCard as Edit, Trash2, Calendar, DollarSign, Package, TrendingUp, Search, Download, ShoppingBag, FileText, Wifi, WifiOff } from 'lucide-react';
...
const Inventory: React.FC = () => {
  ...
  const fetchInventory = async () => {
    try {
      // إذا مافي نت، اقرأ من الذاكرة المحلية
      if (!navigator.onLine) {
        const cached = localStorage.getItem('mahwous_inventory');
        if (cached) {
      ...
      // حفظ البيانات في الذاكرة المحلية للاستخدام عند عدم وجود اتصال
      localStorage.setItem('mahwous_inventory', JSON.stringify(data || []));
    } catch (err) {
      // محاولة استخدام البيانات المخزنة مؤقتاً في حالة فشل الاتصال
      const cached = localStorage.getItem('mahwous_inventory');
      if (cached) {
        setInventoryItems(JSON.parse(cached));
      } else {
    ...
    <div className="p-6 min-h-screen bg-gray-50">
      ...
      {!isOnline && (
        <div className="mb-4 bg-yellow-50 border border-yellow-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <p className="text-yellow-800">أنت غير متصل بالإنترنت. يتم عرض البيانات المخزنة مؤقتاً.</p>
        </div>
      )}
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            {isOnline ? (
              ...
              <span className="flex items-center gap-1 text-yellow-600 text-sm bg-yellow-50 px-2 py-1 rounded-full">
          ...
          <button
            ...
            disabled={!isOnline}
            className={`px-4 py-2 rounded-lg transition-colors duration-200 flex items-center space-x-2 space-x-reverse ${
              isOnline
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex items-center space-x-2 space-x-reverse">
                      <button
                        ...
                        title={!isOnline ? 'غير متاح في وضع عدم الاتصال' : 'تعديل'}
                      >
                      ...
                      {isAdmin && (
                        <button
                          ...
                          title={!isOnline ? 'غير متاح في وضع عدم الاتصال' : 'حذف'}
                        >
