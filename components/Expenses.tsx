  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingExpense) {
        ...
        // خصم المصروف من عهدة الموظف فوراً إذا تم تحديد الخيار
        ...
        // جميع المصروفات معتمدة تلقائياً ومباشرة
        ...
        if (deductFromCustody) {
          ...
          toast.success(
            `تم إضافة المصروف وخصمه من عهدتك بنجاح. الرصيد المتبقي: ${newBalance.toLocaleString('ar-SA')} ر.س`
          );
        ...
        {Math.ceil(filteredExpenses.length / itemsPerPage) > 1 && (
          <div className="bg-gray-50 px-6 py-3 flex items-center justify-between border-t border-gray-200">
            ...
            <div className="flex items-center space-x-2 space-x-reverse">
              ...
              {[...Array(Math.min(5, Math.ceil(filteredExpenses.length / itemsPerPage)))].map((_, i) => {
                const pageNum = i + 1;
                return (
                  <button
                    key={pageNum}
